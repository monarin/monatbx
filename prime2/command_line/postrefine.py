...
def scale_observations_mproc(observations_no, observations_hdlr_ref, iparams, observations_hdlr_set=None, observations_file_set=None):
  ...
  if observations_hdlr_set is not None:
    observations_hdlr, txt_out = postrefine_hdlr.scale_with_reference(observations_hdlr_ref, observations_hdlr=observations_hdlr_set[observations_no], use_binning=False)
  elif observations_file_set is not None:
    observations_hdlr, txt_out = postrefine_hdlr.scale_with_reference(observations_hdlr_ref, filename=observations_file_set[observations_no], use_binning=False)

  observations_hdlr.refresh()
  txt_scale += txt_out
  print txt_scale
...
def scale(observations_hdlr_ref, observations_hdlr, observations_hdlr_set, iparams):
  ...
  txt_scale = ''
  postrefine_hdlr = postrefine_handler(iparams)
  observations_hdlr, txt_out = postrefine_hdlr.scale_with_reference(observations_hdlr_ref, observations_hdlr=observations_hdlr, use_binning=False)
  txt_scale += txt_out

  if observations_hdlr is not None:
    merge_hdlr = merge_handler(iparams)
    observations_hdlr_merge, txt_out = merge_hdlr.merge_observations_handler(observations_hdlr_ref, observations_hdlr, r_final_cutoff=2.0)
    txt_scale += '\n' + txt_out

    for observations_hdlr_in_set in observations_hdlr_set:
      observations_hdlr_in_set.set_params(postref_params=observations_hdlr.postref_params, G_lowres=observations_hdlr.G_lowres)
...
if (__name__ == "__main__"):
  ...
  for i_found_good_cluster in range(len(results)):
    observations_hdlr_ref, observations_hdlr_set, txt_out = results[i_found_good_cluster]
    txt_step2 += txt_out
  ...
  #do first merge
  merge_hdlr = merge_handler(iparams)
  observations_hdlr_scaled_set_merge, observations_hdlr_selected_set, txt_out = merge_hdlr.merge_multi_observations_handler(observations_hdlr_scaled_set, flag_show_summary=True, r_final_cutoff=2.0)
  txt_step2 += txt_out

  """
  #use the first merge to scale all other observations
  ...
  observations_hdlr_scaled_set_merge, observations_hdlr_selected_set, txt_out = merge_hdlr.merge_multi_observations_handler(observations_hdlr_selected_set, flag_show_summary=True, r_final_cutoff=2.0)
  txt_step2 += txt_out
  """
