...
class wilson_plot(object):
  ...
  def __init__(self, f_obs, asu_contents, e_statistics=False):
    ...
    tmp_expected_f_sq = self.expected_f_sq[:]
    self.expected_f_sq *= f_obs_selected.space_group().order_z() \
                        * f_obs_selected.space_group().n_ltr()
    print f_obs_selected.space_group().order_z(), f_obs_selected.space_group().n_ltr()
    for stol_sq, fobs_sq, tmp_exp_f_sq, exp_f_sq in zip(self.mean_stol_sq, self.mean_fobs_sq, tmp_expected_f_sq, self.expected_f_sq):
      print stol_sq, fobs_sq, tmp_exp_f_sq, exp_f_sq

    # fit to straight line
    self.x = self.mean_stol_sq
    self.y = flex.log(self.mean_fobs_sq / self.expected_f_sq)

    for x, y, ratio in zip(self.x, self.y, self.mean_fobs_sq / self.expected_f_sq):
      print x, y, ratio
    ...
    print self.wilson_intensity_scale_factor, self.wilson_k, self.wilson_b
    appl_fobs_sq = self.mean_fobs_sq * (1/self.wilson_intensity_scale_factor) * (1/flex.exp(-2*self.wilson_b*self.mean_stol_sq))
    for appl, exp_fobs in zip(appl_fobs_sq,  self.expected_f_sq):
      print appl, exp_fobs
